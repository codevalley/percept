        ...
        <div class="bg-neutral-400 rounded-[25px] p-7 mb-12 relative">
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2">
              ...
              <transition name="fade-scale" mode="out-in">
                <button 
                  @click="newQuestion.text.trim() ? addQuestion() : suggestQuestion()"
                  :disabled="isSubmitted"
...
export default {
  ...
  setup() {
    ...
    function addQuestion() {
      if (newQuestion.value.text.trim() && !isSubmitted.value) {
        questions.value.push({
          ...
          isAutogenerated: newQuestion.value.isAutogenerated || false
        });
    ...
    function suggestQuestion() {
      ...
      const words = suggestedQuestion.text.split(' ');
      words.forEach((word, index) => {
        setTimeout(() => {
          newQuestion.value.text += (index > 0 ? ' ' : '') + word;
        }, index * 50);
      });
      
      newQuestion.value.response_type = suggestedQuestion.response_type;
      newQuestion.value.isAutogenerated = true;
    }
